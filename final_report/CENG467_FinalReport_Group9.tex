...
\title{Unsupervised Neural Machine Translation for Turkish and English: Denoising Autoencoder and Iterative Back-Translation\thanks{Code repository: \url{https://github.com/alialp5959/ceng467-term-project}}}
\author{Group 9}
...
\begin{table}[htbp]
\centering
\caption{Qualitative translation examples on FLORES-200 devtest comparing Iteration 6 and 7.}
\label{tab:sample-translations}
\resizebox{\textwidth}{!}{
\begin{tabular}{lp{10cm}}
\toprule
\textbf{System / Model} & \textbf{Translation Output} \\
\midrule
\multicolumn{2}{l}{\textbf{Example 1: Turkish to English (TR $\rightarrow$ EN)}} \\
\textit{Source} & Daha önce diyabetik olup şimdi diyabetik olmayan 4 aylık farelerimiz var, diye ekledi. \\
\textit{Reference} & ``We now have 4-month-old mice that are non-diabetic that used to be diabetic,'' he added. \\
\textit{Iter6 Beam 4} & The past before diyabetiks are now diyabetiked by the 4 aylık farelerimiz here, to see the ekledi. \\
\textit{Iter7 Filtered} & The before diyabetiks are now diyabetiked by 4 aylık fares here, to see the ekledi. \\
\textit{mBART-50 (Zero-Shot)} & We've had four months of rats before, which were not diabetic. \\
\midrule
\multicolumn{2}{l}{\textbf{Example 2: Turkish to English (TR $\rightarrow$ EN)}} \\
\textit{Source} & WiFi ile çalışan bir kapı zili yaptığını söyledi. \\
\textit{Reference} & He built a WiFi door bell, he said. \\
\textit{Iter6 Beam 4} & WiFi with the çalışan to kapı zilis yaptığını söyledi. \\
\textit{Iter7 Filtered} & WiFi with the çalışan it kapı zilied out. \\
\midrule
\multicolumn{2}{l}{\textbf{Example 3: English to Turkish (EN $\rightarrow$ TR)}} \\
\textit{Source} & Like some other experts, he is skeptical about whether diabetes can be cured, noting that these findings have no relevance to people who already have Type 1 diabetes. \\
\textit{Reference} & Bazı diğer uzmanlar gibi o da, bu bulguların zaten Tip 1 diyabeti olan kişilerle bir ilişkisi olmadığına dikkat çekerek diyabetin tedavi edilebileceği ile ilgili şüpheci kalmıştır. \\
\textit{Iter6 Beam 4} & Like bazı diğer experts, he is skeptical hakkında whether diabetes can be cured, notları içinde findings have no relevance insanlarda bile Type 1 diabetes. \\
\textit{Iter7 Filtered} & Like bazı diğer experts, hiç skeptical hakkında whether diabetes canı cured, notları içinde findings pek çok relevance insanlar bile Type 1 diabetes. \\
\bottomrule
\end{tabular}
}
\end{table}
...
Beyond copy-like behavior, manual inspection of sampled outputs (with concrete examples shown in Table~\ref{tab:sample-translations}) reveals three recurring error categories:
\begin{itemize}
